...
void ShowSplash (void)
{
  ...
  while( !ended )
  {
	  ...
	  if(WiiStatus.Dev_Net < 0)
		...
		sprintf(debugt, "Updating your romlist! Press any button to continue...");
...
int __checkEmuButton (void)
{
  ...
  if( Element_IsInside(&b_snap, cursor.Left + CURSOR_FIXED_X, cursor.Top + CURSOR_FIXED_Y) )
  {
	if(controls.wpad1.bDown & WPAD_BUTTON_A)
	{
		return STATE_SNAP;
  ...
  if( Element_IsInside(&b_floppy, cursor.Left + CURSOR_FIXED_X, cursor.Top + CURSOR_FIXED_Y) )
  {
	if(controls.wpad1.bDown & WPAD_BUTTON_A)
	{
		return STATE_FLOPPY;
  ...
  if( Element_IsInside(&b_gun, cursor.Left + CURSOR_FIXED_X, cursor.Top + CURSOR_FIXED_Y) )
  {
	if(controls.wpad1.bDown & WPAD_BUTTON_A)
	{
		return STATE_GUN;
  ...
  if( Element_IsInside(&b_snap_load, cursor.Left + CURSOR_FIXED_X, cursor.Top + CURSOR_FIXED_Y) )
  {
	if(controls.wpad1.bDown & WPAD_BUTTON_A)
	{
