...
void draw16bpp_border(void)
{
   ...
   mem_ptr = CPC.scr_base3 + CPC.scr_offs; // PC screen buffer address
...
void draw16bpp_border_double(void)
{
   ...
   colour.w.l = GateArray.palette[16];
   colour.w.h = GateArray.palette[16];
   mem_ptr = (dword *) CPC.scr_base3 + CPC.scr_offs; // PC screen buffer address
   next_line = CPC.scr_bps;
   *mem_ptr =
   *(mem_ptr+next_line) = colour.d; // write one pixel of border colour
   *(mem_ptr+1) =
   *(mem_ptr+next_line+1) = colour.d;
   *(mem_ptr+2) =
   *(mem_ptr+next_line+2) = colour.d;
   *(mem_ptr+3) =
   *(mem_ptr+next_line+3) = colour.d;
   *(mem_ptr+4) =
   *(mem_ptr+next_line+4) = colour.d;
   *(mem_ptr+5) =
   *(mem_ptr+next_line+5) = colour.d;
   *(mem_ptr+6) =
   *(mem_ptr+next_line+6) = colour.d;
   *(mem_ptr+7) =
   *(mem_ptr+next_line+7) = colour.d;
...
void draw16bpp_mode0_double(dword addr)
{
   ...
   val.w.h = GateArray.palette[mode0_table[(idx*2)]];
   *mem_ptr =
   *(mem_ptr+next_line) = val.d; // write one pixels
   *(mem_ptr+1) =
   *(mem_ptr+next_line+1) = val.d;
   val.w.l = GateArray.palette[mode0_table[(idx*2)+1]];
...
void draw16bpp_mode1_double(dword addr)
{
   ...
   val.w.h = GateArray.palette[mode1_table[(idx*4)]];
   *mem_ptr =
   *(mem_ptr+next_line) = val.d; // write one pixels
   val.w.l = GateArray.palette[mode1_table[(idx*4)+1]];
   val.w.h = GateArray.palette[mode1_table[(idx*4)+1]];
   *(mem_ptr+1) =
   *(mem_ptr+next_line+1) = val.d;
   val.w.l = GateArray.palette[mode1_table[(idx*4)+2]];
   val.w.h = GateArray.palette[mode1_table[(idx*4)+2]];
   *(mem_ptr+2) =
   *(mem_ptr+next_line+2) = val.d;
   val.w.l = GateArray.palette[mode1_table[(idx*4)+3]];
...
void draw16bpp_mode2_double(dword addr)
{
   ...
   pat = *(pbRAM + addr); // grab first CPC screen memory byte
   *lobyte = (pat & 0x80) ? pen_on : pen_off;
   *hibyte = (pat & 0x40) ? pen_on : pen_off;
   *mem_ptr =
   *(mem_ptr+next_line) = val.d; // write four pixels
   *lobyte = (pat & 0x20) ? pen_on : pen_off;
   *hibyte = (pat & 0x10) ? pen_on : pen_off;
   *(mem_ptr+1) =
   *(mem_ptr+next_line+1) = val.d;
   *lobyte = (pat & 0x08) ? pen_on : pen_off;
   *hibyte = (pat & 0x04) ? pen_on : pen_off;
   *(mem_ptr+2) =
   *(mem_ptr+next_line+2) = val.d;
   *lobyte = (pat & 0x02) ? pen_on : pen_off;
   *hibyte = (pat & 0x01) ? pen_on : pen_off;
   *(mem_ptr+3) =
   *(mem_ptr+next_line+3) = val.d;
...
void draw16bpp_mode0_half(dword addr)
{
   ...
   idx = *(pbRAM + addr); // grab first CPC screen memory byte
	   ...
	   *(mem_ptr++) = GateArray.palette[mode0_table[(idx<<1)+0]];
	   *(mem_ptr++) = GateArray.palette[mode0_table[(idx<<1)+0]];

	   *(mem_ptr++) = GateArray.palette[mode0_table[(idx<<1)+1]];
	   *(mem_ptr++) = GateArray.palette[mode0_table[(idx<<1)+1]];
...
void draw16bpp_mode1_half(dword addr)
{
   ...
   mem_ptr = CPC.scr_base3 + (CPC.scr_offs/* * CPC.scr_line_offs */); // PC screen buffer address
   idx = *(pbRAM + addr); // grab first CPC screen memory byte
   //idx=idx<<2; NO FUNKA :?
	   *(mem_ptr++) = GateArray.palette[mode1_table[(idx<<2)+0]];
	   *(mem_ptr++) = GateArray.palette[mode1_table[(idx<<2)+1]];
	   *(mem_ptr++) = GateArray.palette[mode1_table[(idx<<2)+2]];
	   *(mem_ptr++) = GateArray.palette[mode1_table[(idx<<2)+3]];
