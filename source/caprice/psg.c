...
inline void Synthesizer_Logic_Q(void)
{
#if 0
   static int nd = 0;
   unsigned int n, spool_ticks;
#endif

   Ton_Counter_A.Hi++;
   if (Ton_Counter_A.Hi >= ((word) (PSG.RegisterAY.TonALo + (PSG.RegisterAY.TonAHi<<8)))) {
      Ton_Counter_A.Hi = 0;
      Ton_A ^= 1;
   }

   Ton_Counter_B.Hi++;
   if (Ton_Counter_B.Hi >= ((word) (PSG.RegisterAY.TonBLo + (PSG.RegisterAY.TonBHi<<8)))) {
      Ton_Counter_B.Hi = 0;
      Ton_B ^= 1;
   }
   Ton_Counter_C.Hi++;
   if (Ton_Counter_C.Hi >= ((word) (PSG.RegisterAY.TonCLo + (PSG.RegisterAY.TonCHi<<8)))) {
      Ton_Counter_C.Hi = 0;
      Ton_C ^= 1;
   }
   Noise_Counter.Hi++;
   ...
   Envelope_Counter.Hi++;
   if (Envelope_Counter.Hi >= ((word) (PSG.RegisterAY.EnvelopeLo + (PSG.RegisterAY.EnvelopeHi<<8)))) {
      Envelope_Counter.Hi = 0;
   }
   #if 0
   if(Ton_Counter_A.Hi && nd < 200){
     nd++;
     fprintf(stderr, "SINTETIZANDO!?! A:%u (%i) B:%u C:%u N:%u (%u) \n", Ton_Counter_A.Hi, ((word) (PSG.RegisterAY.TonALo + (PSG.RegisterAY.TonAHi<<8))) , Ton_Counter_C.Hi, Ton_Counter_C.Hi, Noise_Counter.Hi, Envelope_Counter.Hi);
   }
   if(nd == 190){
     fprintf(stderr, " INDEX ");
     spool_ticks = GetTicks();
     for (n = 0; n < 16; n++) { // loop for all PSG registers
       fprintf(stderr, "%u=%u ", n, PSG.RegisterAY.Index[n]);
     }
     while ((GetTicks()-spool_ticks)<1000);
...
inline void Synthesizer_Mixer_Q(void)
{
   ...
   LevR = LevL;
   if (Ton_EnA) {
      if ((!Envelope_EnA) || (((word) (PSG.RegisterAY.TonALo + (PSG.RegisterAY.TonAHi<<8))) > 4)) {
         k = Ton_A;
      }
      else {
         k = 1;
   ...
   if (Ton_EnB) {
      if ((!Envelope_EnB) || (((word) (PSG.RegisterAY.TonBLo + (PSG.RegisterAY.TonBHi<<8))) > 4)) {
         k = Ton_B;
      }
      else {
         k = 1;
   ...
   if (Ton_EnC) {
      if ((!Envelope_EnC) || (((word) (PSG.RegisterAY.TonCLo + (PSG.RegisterAY.TonCHi<<8))) > 4)) {
         k = Ton_C;
      }
      else {
         k = 1;
...
void Synthesizer_Stereo16(void)
{
   ...
   reg_pair val;
   val.w.h =  Left_Chan / Tick_Counter;
   val.w.l =  Right_Chan / Tick_Counter;


   *(dword *)CPC.snd_bufferptr = val.d;
   *(((dword *)(CPC.snd_bufferptr))+1) = val.d; // write to mixing buffer
