...
do
	...
	$DEVKITPPC/bin/powerpc-gekko-nm "$ELF" | grep __wpads$ | sed -e 's/^\([^ ]*\) .*$/#define __WPADS_ADDR 0x\1/' > $TMPH
	cmp -s trunk/wii/wpad_leds/__wpads.h $TMPH && break
	cp $TMPH trunk/wii/wpad_leds/__wpads.h
        cat trunk/wii/wpad_leds/__wpads.h
